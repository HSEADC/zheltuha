...
document.addEventListener('DOMContentLoaded', function () {
  var tags = document.querySelectorAll('.A_commons_teg');
  var cards = document.querySelectorAll('.M_card');
  var cardsContainer = document.querySelector('.C_archive_cards');
  tags.forEach(function (tag) {
    tag.addEventListener('click', function () {
      var filter = tag.textContent.trim();
      ...
      cards.forEach(function (card) {
        var cardTags = card.querySelectorAll('.A_commons_teg');

        if (Array.from(cardTags).some(function (cardTag) {
          return cardTag.textContent.trim() === filter;
        })) {
          card.style.display = '';
          visibleCardsCount++;
        } else {
      ...
      if (visibleCardsCount > 0 && visibleCardsCount <= 4) {
        cardsContainer.style.gridTemplateColumns = "repeat(".concat(visibleCardsCount, ", 315.55px)");
      } else {
  ...
  var buttons = document.querySelectorAll('.A_button');
  buttons.forEach(function (button) {
    button.addEventListener('click', function () {
      var card = button.closest('.M_card');
      var image = card.querySelector('.A_answer');
      ...
      var allButtonsInContainer = card.querySelectorAll('.A_button');
  ...
  if (window.innerWidth < 1024) {
    var burger = document.getElementsByClassName('A_menu')[0];
    var burgerbig = document.getElementsByClassName('S_nav_bar')[0];
    burger.addEventListener('click', function () {
      if (burgerbig.style.height === '17.52vw' || burgerbig.style.height === '') {
  ...
  var buttons2 = document.querySelectorAll('.A_button');
  buttons2.forEach(function (button) {
    button.addEventListener('click', function (event) {
      var buttonText = button.textContent.trim().toLowerCase();
      var cardContainer = document.querySelector('.O_test_page_right_cards');
